...
def load_clusters(path):
    clusters, cid = defaultdict(list), -1
...
def build_bert(vocab_size, max_len, save_dir):
    cfg = BertConfig(vocab_size=vocab_size, hidden_size=256,
                     num_hidden_layers=6, num_attention_heads=4,
                     intermediate_size=1024, max_position_embeddings=max_len)
    BertModel(cfg).save_pretrained(save_dir)
...
def sanity_checks(model, strands):
    ...
    clusters = util.community_detection(emb, min_community_size=100, threshold=0.92)
    sizes = [len(c) for c in clusters]
    import numpy as np
    print(f"probe groups {len(clusters)} median {np.median(sizes):.0f} max {max(sizes)}\n")
...
def train(cfg):
    ...
    build_bert(9, cfg.max_len, tok_dir)
